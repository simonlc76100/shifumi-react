...
export default function MatchDisplay({ idParam }) {
  ...
  const getMatch = useCallback(async () => {
    const response = await fetch(`http://fauques.freeboxos.fr:3000/matches/${idParam}`, {
      method: "GET",
      headers: {
  ...
  useEffect(() => {
    const eventSource = new EventSourcePolyfill(
      `http://fauques.freeboxos.fr:3000/matches/${idParam}/subscribe`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  async function handleMove(move) {
    ...
    const response = await fetch(
      `http://fauques.freeboxos.fr:3000/matches/${match._id}/turns/${idTurn}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
